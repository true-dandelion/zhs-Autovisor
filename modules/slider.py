...
async def slider_verify(page: Page, modules: list[ModuleType], log_callback=None):
    global cv2, np
    np, cv2 = modules
    ...
    if not cv2 or not np:
        log("OpenCV或Numpy导入失败,无法开启自动滑块验证.", "warn")
        return
    ...
    for x in range(0, 3):
        try:
            ...
            await page.wait_for_selector(".yidun_bg-img", state="attached")
            log(f"第{x + 1}次尝试过滑块验证...")
