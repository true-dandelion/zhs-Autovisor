    ...
    if is_hike_class:
        ...
        if include_all:
            ...
            for each in all_class:
                isDone = await each.locator(".icon-finish").count() > 0
                if not isDone:
                    to_learn_class.append(each)
            logger.write_log(f"Get to-learn class: {len(to_learn_class)}\n")
        ...
        if include_all:
            ...
            for each in all_class:
                if is_new_version:
                    progress_elem = each.locator(".progress-num")
                    if await progress_elem.count() > 0:
                        progress = await progress_elem.text_content()
                        isDone = progress == "100%"
                    else:
                        isDone = await each.locator(".time_icofinish").count() > 0
                else:
                    isDone = await each.locator(".time_icofinish").count() > 0
                if not isDone:
                    to_learn_class.append(each)
